...
describe("formatParagraph", () => {
	...
	beforeAll(() => {
		defaultEnabledFormatters = registeredFormatters.filter((formatter: ParagraphFormatter) => formatter.isEnabled(DEFAULT_SETTINGS));
	})
	...
	it("should remove scripture reference links if the setting to retain them is disabled", () => {
		...
		// Act
		const result = formatParagraph(input, enabledFormatters);
